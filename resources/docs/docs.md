...
The `home-routes` handler in the `clj-guestbook.routes.home` namespace
...
The `home-page` function will in turn call the `clj-guestbook.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clj-guestbook.handler` namespace:
...
Request middleware functions are located under the `clj-guestbook.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clj-guestbook.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `clj-guestbook.db.core` in some other namespace.
